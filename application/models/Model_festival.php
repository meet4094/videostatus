...
class Model_festival extends CI_Model
{
    ...
    function getCategories($param)
    {
        ...
        $iQuery = "SELECT id,name,image,festival_date,active_from,detail_display,detail_message,created_date,modified_date,is_del as status FROM sh_festivalcategory {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iCategories['data']) && count($iCategories['data']) > 0) {
            ...
            foreach ($iCategories['data'] as $key => $iCategory) {
                ...
                if (isset($iCategory['image']) && !empty($iCategory['image']))
                    ...
                    $iCategories['data'][$key]['image_url'] = FESTIVALCATEGORY_IMAGE_URL . DS . $iCategory['image'];
            }
        }
    ...
    function getImages($param)
    {
        ...
        $iQuery = "SELECT id,name,description,category_id,image,language_id,created_date,modified_date,is_del as status FROM sh_festivalimage {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iImages['data']) && count($iImages['data']) > 0) {
            ...
            foreach ($iImages['data'] as $key => $iImage) {
                ...
                if (isset($iImage['image']) && !empty($iImage['image']))
                    ...
                    $iImages['data'][$key]['image_url'] = FESTIVALIMAGE_IMAGE_URL . DS . $iImage['image'];
            }
        }
    ...
    function getVideos($param)
    {
        ...
        $iQuery = "SELECT id,name,description,category_id,image,language_id,created_date,modified_date,is_del as status FROM sh_festivalvideo {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iVideos['data']) && count($iVideos['data']) > 0) {
            ...
            foreach ($iVideos['data'] as $key => $iVideo) {
                ...
                if (isset($iVideo['image']) && !empty($iVideo['image']))
                    ...
                    $iVideos['data'][$key]['image_url'] = FESTIVALVIDEO_IMAGE_URL . DS . $iVideo['image'];
            }
        }
