...
class Model_business extends CI_Model {
    ...
    function getCategories($param) {
        ...
        $iQuery = "SELECT id,name,image,detail_display,detail_message,created_date,modified_date,is_del as status FROM sh_businesscategory {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iCategories['data']) && count($iCategories['data']) > 0) {
            ...
            foreach ($iCategories['data'] as $key => $iCategory) {
                ...
                if (isset($iCategory['image']) && !empty($iCategory['image']))
                    ...
                    $iCategories['data'][$key]['image_url'] = BUSINESSCATEGORY_IMAGE_URL.DS.$iCategory['image'];
    ...
    function getImages($param) {
        ...
        $iQuery = "SELECT id,name,description,category_id,image,language_id,created_date,modified_date,is_del as status FROM sh_businessimage {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iImages['data']) && count($iImages['data']) > 0) {
            ...
            foreach ($iImages['data'] as $key => $iImage) {
                ...
                if (isset($iImage['image']) && !empty($iImage['image']))
                    ...
                    $iImages['data'][$key]['image_url'] = BUSINESSIMAGE_IMAGE_URL.DS.$iImage['image'];
    ...
    function getVideos($param) {
        ...
        $iQuery = "SELECT id,name,description,category_id,image,language_id,created_date,modified_date,is_del as status FROM sh_businessvideo {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        if (isset($iVideos['data']) && count($iVideos['data']) > 0) {
            ...
            foreach ($iVideos['data'] as $key => $iVideo) {
                ...
                if (isset($iVideo['image']) && !empty($iVideo['image']))
                    ...
                    $iVideos['data'][$key]['image_url'] = BUSINESSVIDEO_IMAGE_URL.DS.$iVideo['image'];
