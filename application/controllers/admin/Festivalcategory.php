...
class Festivalcategory extends MY_Controller
{
    ...
    function view()
    {
        $this->load->view("admin/festivalcategory_view");
    }
    ...
    function add()
    {
        if (isset($this->param['submit'])) {
            ...
            $status = $this->model_api->add("sh_festivalcategory", $inserted_fields);
            if (!isset($status['data']['id']) || $status['statuscode'] != 1) {
                ...
                if (isset($_FILES['image']['error']) && $_FILES['image']['error'] == 0) {
                    ...
                    $this->model_api->update("sh_festivalcategory", $updated_fields, array("and" => array("id" => $id)));
                }
                $res = success_res();
            }
            $this->load->view("admin/festivalcategory_add", $res);
        } else {
            $this->load->view("admin/festivalcategory_add");
        }
    }
    ...
    function edit($id)
    {
        if (isset($this->param['submit'])) {
            if (isset($_FILES['image']['error']) && $_FILES['image']['error'] == 0) {
                $iOriginal = $this->model_api->get("sh_festivalcategory", array("image"), array("and" => array("id" => $id)));
            ...
            $status = $this->model_api->update("sh_festivalcategory", $inserted_fields, array("and" => array("id" => $id)));
            if (!isset($status['statuscode']) || $status['statuscode'] != 1) {
                $res = error_res();
            } else {
                $res = success_res();
            }
            $edit_festivalcategory = $this->model_api->get("sh_festivalcategory", array("id,name,festival_date,active_from,detail_display,detail_message,image,is_del"), array("and" => array("id" => $id)));
            $res['edit_festivalcategory'] = $edit_festivalcategory['data'][0];
            $this->load->view("admin/festivalcategory_edit", $res);
        } else {
            $edit_festivalcategory = $this->model_api->get("sh_festivalcategory", array("id,name,festival_date,active_from,detail_display,detail_message,image,is_del"), array("and" => array("id" => $id)));
            $res['edit_festivalcategory'] = $edit_festivalcategory['data'][0];
            $this->load->view("admin/festivalcategory_edit", $res);
    ...
    function delete($id)
    {
        if ($id && is_numeric($id)) {
            ...
            $iQry = "DELETE FROM sh_festivalcategory WHERE id = {$id}";
            $this->model_api->execute_query($iQry);
        }
        redirect(BASE_URL . "admin/festivalcategory/view");
    }
    ...
    function load_festivalcategory()
    {
        ...
        $iResCount = $this->model_api->execute_query("SELECT COUNT(id) cnt FROM sh_festivalcategory " . $iWhere);
        ...
        $iQry = "SELECT id,name,image,festival_date,active_from,detail_display,detail_message,created_date,is_del FROM sh_festivalcategory {$iWhere} ORDER BY id DESC {$iLimit}";
        ...
        foreach ($iDataLists['data'] as $key => $iData) {
            ...
            $records["data"][] = array(
                ...
                '<a href="' . BASE_URL . "admin/festivalcategory/edit/" . $iData['id'] . '" class="btn btn-sm btn-outline blue"><i class="fa fa-edit"></i> Edit</a> <a href="' . BASE_URL . "admin/festivalcategory/delete/" . $iData['id'] . '" class="btn btn-sm btn-outline red"><i class="fa fa-trash"></i> Delete</a>',
            );
        }
